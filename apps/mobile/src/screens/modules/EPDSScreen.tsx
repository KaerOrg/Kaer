...
import { getAllEPDSEntries, deleteEPDSEntry, type EPDSEntry } from '../../lib/database'
...
export default function EPDSScreen() {
  ...
  const [entries, setEntries] = useState<EPDSEntry[]>([])
  const [loading, setLoading] = useState(true)

  useFocusEffect(
    useCallback(() => {
      let active = true
      getAllEPDSEntries().then(data => {
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.header}>
          <Text style={typography.h2}>EPDS</Text>
        </View>

        <Pressable
          style={[styles.newBtn, isTeenMode && accentColor && { backgroundColor: accentColor }]}
          onPress={() => navigation.navigate('EPDSEntry', {})}
        >
