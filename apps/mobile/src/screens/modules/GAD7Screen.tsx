...
import { getAllGAD7Entries, deleteGAD7Entry, type GAD7Entry } from '../../lib/database'
...
export default function GAD7Screen() {
  ...
  const [entries, setEntries] = useState<GAD7Entry[]>([])
  const [loading, setLoading] = useState(true)

  useFocusEffect(
    useCallback(() => {
      let active = true
      getAllGAD7Entries().then(data => {
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.header}>
          <Text style={typography.h2}>GAD-7</Text>
        </View>

        <Pressable
          style={[styles.newBtn, isTeenMode && accentColor && { backgroundColor: accentColor }]}
          onPress={() => navigation.navigate('GAD7Entry', {})}
        >
