...
import { saveASRS18Entry, generateId } from '../../lib/database'
import { ASRS18_DATA, computeASRS18SubScores } from '../../data/asrs18'
...
const ITEMS = ASRS18_DATA.items
const OPTIONS = ASRS18_DATA.options
const TOTAL = ITEMS.length // 18

// Indices de début de chaque partie
const PART_A_END = 6
...
export default function ASRS18EntryScreen() {
  ...
  const handleSubmit = async () => {
    ...
    const subScores = computeASRS18SubScores(answersAsNumbers)
    await saveASRS18Entry({
      id: generateId(),
      answers: answersAsNumbers,
      sub_scores: subScores,
      total_score: subScores.part_a + subScores.part_b,
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.instructions}>
          <Text style={styles.instructionTitle}>Ces 6 derniers mois,</Text>
          <Text style={styles.instructionText}>
            à quelle fréquence les situations suivantes vous sont-elles arrivées ?
          </Text>
        </View>

        {/* Questions — Partie A */}
        <View style={styles.questions}>
          <SectionHeader label="Partie A" />
          {ITEMS.slice(0, PART_A_END).map((item, i) => (
            <QuestionRow
              key={i}
              index={i}
          ...
          {/* Questions — Partie B */}
          <SectionHeader label="Partie B" />
          {ITEMS.slice(PART_A_END).map((item, i) => (
            <QuestionRow
              key={i + PART_A_END}
              index={i + PART_A_END}
              question={item.question}
              value={answers[i + PART_A_END]}
              accentColor={accentColor}
              onChange={v => setAnswer(i + PART_A_END, v)}
            />
          ))}
        </View>
