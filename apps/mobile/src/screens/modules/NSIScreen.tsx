...
import { getAllNSIEntries, deleteNSIEntry, type NSIEntry } from '../../lib/database'
...
export default function NSIScreen() {
  const navigation = useNavigation<Nav>()
  const { isTeenMode, tt, teenColor } = useTeen()
  const accentColor = teenColor('nsi')

  const [entries, setEntries] = useState<NSIEntry[]>([])
  const [loading, setLoading] = useState(true)

  useFocusEffect(
    useCallback(() => {
      let active = true
      getAllNSIEntries().then(data => {
                ...
                <View style={styles.cardMain}>
                  ...
                  <View style={styles.scoreRow}>
                    ...
                    <Text style={[
                      ...
                      {entry.score}
                      <Text style={styles.scoreMax}> / 45</Text>
                    </Text>
                  </View>
                  {entry.recurrent_pct !== null && (
                    <Text style={styles.metaText}>
                      {entry.recurrent_pct}{'%'} récurrents
                    </Text>
                  )}
                </View>
                <Pressable
