...
describe('TechniqueRow', () => {
  it('affiche le nom, le bénéfice et le rythme compact', () => {
    const { getByText } = render(<TechniqueRow technique={TECHNIQUE} lbl={lbl} onOpen={jest.fn()} onInfo={jest.fn()} />)
  ...
  it('remonte la clé de la technique au tap sur la ligne entière', () => {
    ...
    const { getByTestId } = render(
      <TechniqueRow technique={TECHNIQUE} lbl={lbl} onOpen={onOpen} onInfo={jest.fn()} />,
    )
    fireEvent.press(getByTestId('breathing-technique-diaphragmatique'))
    expect(onOpen).toHaveBeenCalledWith('diaphragmatique')
  })

  it('ouvre « En savoir plus » depuis la ligne, sans lancer de session', () => {
    const onOpen = jest.fn()
    const onInfo = jest.fn()
    const { getByTestId } = render(
      <TechniqueRow technique={TECHNIQUE} lbl={lbl} onOpen={onOpen} onInfo={onInfo} />,
    )
    fireEvent.press(getByTestId('breathing-info-diaphragmatique'))
    expect(onInfo).toHaveBeenCalledWith('diaphragmatique')
    expect(onOpen).not.toHaveBeenCalled()
  })

  it('n’affiche aucune preuve scientifique', () => {
    const { queryByText } = render(<TechniqueRow technique={TECHNIQUE} lbl={lbl} onOpen={jest.fn()} onInfo={jest.fn()} />)
