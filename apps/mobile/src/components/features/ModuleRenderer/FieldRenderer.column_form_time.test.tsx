...
// Une seule colonne contenant deux ancres (structure réelle du seed chronobiologie).
const ONE_COLUMN_TWO_ANCHORS = makeField({
  id: 'chrono.col.h', section_id: 'chrono.anchors', sort_order: 10,
  text_code: 'modules.chrono_bio.section_anchors',
  props: { color: '#3B82F6', step_number: '1' },
  children: [
    makeField({
      id: 'chrono.col.wake', section_id: 'chrono.anchors', parent_field_id: 'chrono.col.h',
      field_type: 'column_time_field', sort_order: 11,
      text_code: 'modules.chrono_bio.wake_time', props: { key: 'wake_time', optional: '1' },
    }),
    makeField({
      id: 'chrono.col.bed', section_id: 'chrono.anchors', parent_field_id: 'chrono.col.h',
      field_type: 'column_time_field', sort_order: 15,
      text_code: 'modules.chrono_bio.bedtime', props: { key: 'bedtime', optional: '1' },
    }),
  ],
})

const FILTER_FIELDS: ContentField[] = [
  makeField({ id: 'chrono.cfg', field_type: 'column_form_config', sort_order: 0, props: {} }),
  ONE_COLUMN_TWO_ANCHORS,
]

function renderWithConfig(patientConfig: Record<string, unknown> | null) {
  return render(
    <FieldRenderer
      preview_kind="column_form"
...
describe('column_form — filtrage des ancres (config.anchors)', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    ;(database.getAllFormEntries as jest.Mock).mockResolvedValue([])
  })

  it('n\'affiche que les ancres sélectionnées par le praticien', async () => {
    renderWithConfig({ anchors: ['wake_time'] })
    fireEvent.press(await screen.findByTestId('new-entry'))
    expect(await screen.findByTestId('time-wake_time')).toBeTruthy()
    expect(screen.queryByTestId('time-bedtime')).toBeNull()
  })

  it('affiche toutes les ancres si config.anchors est vide (défaut)', async () => {
    renderWithConfig({ anchors: [] })
    fireEvent.press(await screen.findByTestId('new-entry'))
    expect(await screen.findByTestId('time-wake_time')).toBeTruthy()
    expect(screen.getByTestId('time-bedtime')).toBeTruthy()
  })

  it('affiche toutes les ancres si patientConfig est null', async () => {
