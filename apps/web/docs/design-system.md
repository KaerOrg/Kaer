...
> **Moteur d'agrégation & trous (`lib/chartAggregation.ts`, #159).** `aggregateByCadence(points, 'weekly'|'monthly', rangeDays)` transforme des saisies brutes datées en une série continue d'unités de cadence (1 point/unité, moyenne + `n`, `null` pour une unité vide). `computeGapSegments(series)` en dérive les `bridges` (trou d'1 unité) et `bands` (2 unités vides ou +), à passer en prop `gaps` de `TrendChart`. Le pont `buildCadenceTrend(points, cadence, rangeDays)` combine les deux en une passe (`{ data, gaps }`) — c'est la « courbe lissée à la cadence » branchée dans la frise Humeur, le panneau Sommeil (`SleepDataPanel`) **et le panneau Activation** (`BehavioralActivationPanel` : deux `TrendChart` empilés Plaisir/Accomplissement, chacun sa politique de trous) de la page Évolution (agrégée par défaut, bascule « voir chaque saisie » pour les points bruts ; stats et delta restent sur le brut). Pour les vues sans sélecteur de période (modale « Données »), `spanDays(points)` donne la fenêtre couvrant tout l'historique afin qu'aucune saisie ancienne ne soit écartée. Seuls les modules de cadence `weekly` (auto-relevés quotidiens : sommeil, humeur, activation) sont agrégés ; les instruments discrets (`per_session` peur, `per_passation` échelles) gardent un point par événement. La **config par type** (`clinicalChartConfig.ts` → `MODULE_EVOLUTION_CONFIG` / `moduleEvolutionConfig(type)`) fixe unité, bornes Y, cadence, métrique d'aperçu et `kind` (`metric` / `fingerprint` / `scale` / `text`) par module. Logique pure testée.
...
| `filters` | `ReactNode` | Contrôles de filtrage (recherche, segments, dropdowns…). Rendus dans un **panel encadré** (`.data-table__filters` : bordure + radius + fond blanc surface + padding, 100 % tokens). Le cadre n'apparaît **que si** `filters` est fourni (pas de panel vide) |
