...
function fields(): ContentField[] {
  return [
    ...
    field('exercise_config', { id: 'cfg', props: { duration_1: '7', duration_2: '20' }, sort_order: 3 }),
...
describe('CrisisCompanionLayout (aperçu web)', () => {
  ...
  it('affiche les durées configurées (clés indexées) + message de fin neutre', () => {
    const { container } = render(<CrisisCompanionLayout fields={fields()} t={t} moduleId="distress_tolerance" />)
    const durations = container.querySelectorAll('.cc-duration')
    expect(durations).toHaveLength(2)
    expect(durations[0].textContent).toContain('7')
    expect(durations[1].textContent).toContain('20')
