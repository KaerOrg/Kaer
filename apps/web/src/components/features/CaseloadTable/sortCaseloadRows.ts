...
export type CaseloadSortColumn = 'patient' | 'status' | 'waiting' | 'alert'
 ...
 * première fois (avant de pouvoir basculer asc/desc). Ex. « En attente » montre
 * d'emblée les dossiers ayant le plus d'attentes en tête.
 */
 ...
 * uniquement (A→Z, actif→archivé, 0→N attentes, critique→ok).
 */
