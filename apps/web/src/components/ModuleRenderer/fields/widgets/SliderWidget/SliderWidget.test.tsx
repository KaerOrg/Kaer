...
describe('SliderWidget', () => {
  ...
  it('applique min et max depuis le spec', () => {
    const { container } = render(<SliderWidget spec="slider:0:120:min" />)
    const input = container.querySelector('input') as HTMLInputElement
  ...
  it("n'affiche pas d'unité quand elle est absente", () => {
    const { container } = render(<SliderWidget spec="slider:0:3" />)
  ...
  it('valeur médiane (1–10 → 6)', () => {
    const { container } = render(<SliderWidget spec="slider:1:10" />)
