...
describe('timesOverlap', () => {
  it('chevauchement partiel (9h-12h vs 10h-13h)', () =>
    expect(timesOverlap('09:00', '12:00', '10:00', '13:00')).toBe(true))
  it('chevauchement contenant (9h-18h contient 10h-12h)', () =>
    expect(timesOverlap('09:00', '18:00', '10:00', '12:00')).toBe(true))
  it('plages contiguës non chevauchantes (9h-12h vs 12h-18h)', () =>
    expect(timesOverlap('09:00', '12:00', '12:00', '18:00')).toBe(false))
  it('plages disjointes (9h-12h vs 14h-18h)', () =>
    expect(timesOverlap('09:00', '12:00', '14:00', '18:00')).toBe(false))
  it('tolère le format "HH:MM:SS" en entrée', () =>
    expect(timesOverlap('09:00:00', '12:00:00', '10:00:00', '13:00:00')).toBe(true))
})
...
describe('computeAvailableSlots', () => {
  ...
  it('applique le buffer entre créneaux (50 min RDV + 10 min buffer = 60 min par slot)', () => {
    // 09:00-12:00 = 180 min. Avec 50+10=60 min/slot → 3 slots
    const rule: AvailabilityRule = { ...baseRule, buffer_minutes: 10 }
    const slots = computeAvailableSlots([rule], [], [], '2024-01-08')
    expect(slots).toHaveLength(3)
    expect(slots[0].starts_at).toBe('2024-01-08T09:00:00')
    expect(slots[0].ends_at).toBe('2024-01-08T09:50:00')
    expect(slots[1].starts_at).toBe('2024-01-08T10:00:00')
    expect(slots[2].starts_at).toBe('2024-01-08T11:00:00')
  })

  it('buffer=0 se comporte comme sans buffer', () => {
    const rule: AvailabilityRule = { ...baseRule, buffer_minutes: 0 }
    const slots = computeAvailableSlots([rule], [], [], '2024-01-08')
    expect(slots).toHaveLength(3)
    expect(slots[1].starts_at).toBe('2024-01-08T09:50:00')
  })

  it('deux règles matin + après-midi : créneaux triés chronologiquement', () => {
    const morning: AvailabilityRule = { ...baseRule, id: 'r1', start_time: '09:00', end_time: '12:00' }
    ...
    // 09:00-12:00 / 50 min = 3 slots, 14:00-16:00 / 50 min = 2 slots → 5
    expect(slots).toHaveLength(5)
    expect(slots[0].starts_at).toBe('2024-01-08T09:00:00')
    expect(slots[2].starts_at).toBe('2024-01-08T10:40:00')
    expect(slots[3].starts_at).toBe('2024-01-08T14:00:00')
    expect(slots[4].starts_at).toBe('2024-01-08T14:50:00')
  })
  ...
  it('exception avec horaires explicites remplace toutes les règles du jour', () => {
    ...
    const slots = computeAvailableSlots([morning, afternoon], [exception], [], '2024-01-08')
    // 10:00-11:00 / 50 min = 1 seul slot, après-midi ignoré
    expect(slots).toHaveLength(1)
    expect(slots[0].starts_at).toBe('2024-01-08T10:00:00')
  })
})
...
describe('saveAvailabilityRule', () => {
  it('retourne ok=true avec la règle créée', async () => {
    ...
    const result = await saveAvailabilityRule({
      ...
      start_time: '09:00', end_time: '12:00', slot_duration_minutes: 50, buffer_minutes: 0,
    })
...
describe('createAppointment', () => {
  ...
  it('crée un RDV en statut pending (le patient doit confirmer)', async () => {
    const appt = { id: 'a1', ...params, status: 'pending', notes: null, created_at: '', updated_at: '' }
