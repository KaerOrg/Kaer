...
-- ============================================================
-- Idempotent : DELETE (cascade sur les blocs) + INSERT, même idiome que
-- psyedu_seed.sql. Le rebuild reflète le seed à chaque ré-exécution, y compris
-- la SUPPRESSION d'un bloc retiré ici (ce qu'un `ON CONFLICT DO UPDATE` ne fait
-- pas : il n'efface jamais une ligne devenue absente du seed). Voir ticket #203.

-- ─── breathing_techniques ────────────────────────────────────────────────────

-- Purge du topic guide (cascade → ses psyedu_blocks) puis reconstruction complète.
DELETE FROM psyedu_topics WHERE module_key = 'breathing_techniques' AND topic_key = 'guide';
