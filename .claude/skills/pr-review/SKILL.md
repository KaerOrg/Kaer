...
## ⭐ Le cœur de la review — 4 principes, avant toute checklist

> **Ces quatre principes priment sur tout le reste. Les RULES mécaniques qui suivent
> ne sont que des détecteurs au service de ces principes — jamais un substitut au
> jugement.** Une PR qui passe tous les `grep` mais viole un de ces principes est à
> reprendre. Pose-toi ces questions sur **chaque fichier**, et ne conclus jamais
> « conforme » sur la seule foi d'une recherche de motif.

1. **Séparation des concepts — zéro métier dans le design system.** `ui/` ne connaît
   ni patient, ni module, ni échelle, ni service, ni store, ni contexte métier : tout
   entre par props. Un composant `ui/` qui importe un `*Service`, un store, un contexte,
   un **type de persistance** (`lib/database`, une row Supabase) ou hardcode une clé
   i18n de domaine (`scales.*`, `modules.*`, `file_active.*`) est une **fuite métier
   dans le design system → violation bloquante**. Vérifie-le en **lisant les imports
   de chaque composant `ui/`**, pas seulement en cherchant `supabase`.

2. **Découpage / couches — la donnée se possède en haut, s'injecte vers le bas.** Une
   feuille (cellule, panneau, ligne) ne va pas chercher ni muter ses propres données :
   elle reçoit `données + callbacks` par props ; le fetch/mutation/auth/toast vivent à
   la page. **Lis chaque enfant d'un conteneur** : si une feuille fait son `fetchX`/
   `createX` via un service alors que ses frères remontent leur état, c'est une rupture
   de couches → violation (cf. RULE dédiée plus bas).

3. **Qualité du code.** Types stricts (zéro `any`/`as unknown`/suppression), hooks
   corrects, zéro allocation inline, état dérivé pendant le render, nommage juste,
   composants mémoïsés là où il faut. Le code se lit comme le code voisin.

4. **Simplicité.** La solution la plus simple qui marche. Pas de composant créé quand
   un primitive existe (étendre > dupliquer), pas d'état quand une dérivation suffit,
   pas d'abstraction spéculative, pas de prop morte, pas de couche inutile. Si c'est
   plus compliqué que le besoin, c'est une remarque.

> **Méthode anti-angle-mort :** un `grep` propre (`supabase`/`db` absent) ne prouve
> RIEN sur la séparation — un composant peut passer par un service et rester une fuite.
> Toujours **ouvrir et lire** les imports + le corps de chaque composant `ui/` et de
> chaque feuille avant de statuer.
...
> **Le grep `supabase`/`db` ne suffit PAS — il faut LIRE chaque composant.** Un
> composant peut respecter cette règle à la lettre (il passe par un service) tout en
> **embarquant de la logique métier et l'orchestration de ses propres données** —
> ce qui est tout aussi inacceptable. Ne jamais conclure « accès données conforme »
> sur la seule foi d'un grep propre.

#### RULE — Logique métier / orchestration de données dans un composant (au-delà du grep)
*(source : coding-standards.md § "Architecture en couches" — « UI : affichage uniquement — zéro logique métier »)*

> **Procédure obligatoire — ne pas s'arrêter au grep `supabase`/`db`.** Pour tout
> composant « conteneur » (page, panneau, ligne, détail dépliable, onglet), **lire
> chaque composant enfant qu'il rend**. Un enfant qui passe par un service mais
> pilote son propre cycle de données est le trou exact par lequel ce genre de cas
> passe (`grep` propre ⇒ faux sentiment de conformité).

Pour **chaque composant** (et en priorité les feuilles rendues par un détail/panneau/ligne), chercher :

- **Import d'un `*Service` + fetch/mutation pilotés en interne** : le composant appelle `fetchX(...)` dans un `useEffect`, et/ou `createX/updateX/deleteX(...)` dans un handler, **possède son propre état de données** (`useState<Row[]>`), et lit l'identité acteur (`useAuthStore().practitioner`) / déclenche `useToast` lui-même.
  → **Violation** dès que ce composant est une **feuille** dont les **frères remontent leur état** au parent par props. Le composant doit redevenir **présentationnel** : recevoir **données + callbacks par props** (`items`, `onLoad`, `onAdd`, `onPatch`, `onDelete`), l'orchestration (fetch, mutation, auth, toast) restant **possédée par la page**.
  → Dans un composant `components/ui/` : **violation bloquante systématique** (un primitive ne va jamais chercher ses données — cf. Étape 4.4).

- **Incohérence de pattern dans une même feature** : deux données de même nature (ex. `actions`, `waits`, `notes` d'un même dossier) suivies par **deux architectures différentes** — l'une remontée à la page et injectée par props, l'autre fetchée/mutée dans une feuille → **violation** (cohérence). Aligner sur le pattern « état remonté à la page ».

**Test décisif :** « cet enfant reçoit-il ses données et ses actions par props, comme ses frères, ou se les procure-t-il lui-même ? » S'il se les procure lui-même alors que la feature remonte l'état ailleurs → le métier est *dans* la feuille au lieu d'être *injecté*.

> Une exception (chargement paresseux par ligne assumé pour la scalabilité) reste
> possible, **mais** le `load`/`add` doit alors passer par des **props callbacks**
> injectées par la page — jamais `useAuthStore`/`useToast`/service *dans* la feuille —
> et la dérogation doit être **justifiée en JSDoc**.
...
## Étape 5 — Design system d'abord : réutiliser, sinon étendre, jamais redupliquer
...
- [ ] **Lu chaque enfant de chaque conteneur** (panneau/détail/ligne) — aucune feuille ne pilote son propre fetch/mutation via un service quand ses frères remontent leur état par props (au-delà du grep `supabase`/`db`)
