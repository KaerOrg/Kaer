...
| `fear_thermometer` | Thermomètre de la peur | Implémenté — **parcours d'exposition unifié** (`preview_kind = 'exposure_tracker'`, layout reconstruit en 4 modes : échelle de la peur → marches classées par SUDS cible → détail courbe+historique → exposition enrichie). Modèle *expérience* (inhibitory learning) : prédiction « ce que je redoute » → pic → résultat « ce qui s'est passé ». SUDS anticipé/pic/final 0–100, stratégies DB-driven, `DesensitizationChart` par marche. SQLite `fear_situations`/`fear_entries` (+ colonnes `suds_peak`/`expectation_text`/`outcome_text`). **Aperçu praticien web refondu** (parcours interactif échelle→détail courbe SVG→formulaire, mock déterministe, niveau `SliderDashboardLayout`). Spec : [`docs/spec/exposure-journey.md`](docs/spec/exposure-journey.md) |
