...
- [x] Droits patient RGPD (`feat/rgpd-droits-patient`, ticket #27, épic conformité #29) — export (art. 15/20) & effacement complet (art. 17). RPC `SECURITY DEFINER` `export_patient_data` (agrège ~18 tables en JSON brut, valeurs brutes uniquement — MDR) + `erase_patient_data` (trace l'effacement + supprime le non-cascadant : `invitations` par email, `caseload_entries`) gardés par `fn_can_access_patient` (`auth.uid()` → patient lui-même ou praticien lié). Suppression du compte `auth.users` via Edge Function `delete-patient-account` (service_role) → cascade `patients` + ~20 tables. **Web** : service `patientDataRightsService.ts` + bloc `PatientDataRights` (fiche patient, export JSON téléchargé + effacement avec re-saisie du nom). **Mobile** : self-service `ProfileScreen` (export via `Share`, suppression de compte) + purge SQLite locale `purgeAllLocalData()` + purge au login si compte effacé. Tracé dans `access_audit_log` (`export`/`erase`). Docs : [`docs/rgpd-droits-patient.md`](docs/rgpd-droits-patient.md)
- [ ] Notifications push
