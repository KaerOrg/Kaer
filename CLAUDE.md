...
| `fear_thermometer` | **Exposition graduée** (ex-« Thermomètre de la peur ») | Implémenté — **parcours d'exposition unifié** (`preview_kind = 'exposure_tracker'`, layout reconstruit en 4 modes : échelle de la peur → marches classées par niveau de stress initial → détail courbe+historique → exposition enrichie). Modèle *expérience* (inhibitory learning) : prédiction « ce que je redoute » → pic → résultat « ce qui s'est passé ». Stress anticipé/pic/final 0–100, stratégies DB-driven, `DesensitizationChart` par marche. SQLite `fear_situations`/`fear_entries` (+ colonnes `suds_peak`/`expectation_text`/`outcome_text`). **Aperçu praticien web refondu** (parcours interactif échelle→détail courbe SVG→formulaire, mock déterministe, niveau `SliderDashboardLayout`). **A absorbé l'ancien module `exposure_hierarchy`** (retiré). Spec : [`docs/spec/exposure-journey.md`](docs/spec/exposure-journey.md) |
...
- [x] Refonte Thermomètre de la peur → **Exposition graduée** (`fear_thermometer`) — **parcours d'exposition unifié mobile** inspiré des outils de référence (Mayo Clinic Anxiety Coach, MindShift CBT, Exposure: Face Your Fears). Layout `exposure_tracker` reconstruit en 4 modes (échelle → marches → détail courbe+historique → exposition enrichie), saisie *expérience* (prédiction/pic/résultat), 3 colonnes ajoutées à `fear_entries` (`suds_peak`, `expectation_text`, `outcome_text`, migration idempotente), composants éclatés (LadderList/StepFormView/StepDetail/ExposureForm/SudsField/SudsBar/SessionCard + hook + logique pure), i18n fr/en + teen, teen mode, bandeau MDR, 21 tests Jest. **Aperçu praticien web** refondu au niveau `SliderDashboardLayout` : orchestrateur interactif `ExposureTrackerLayout` (échelle → détail courbe SVG `DesensitizationChartPreview` → formulaire enrichi), mock déterministe `exposureMock.ts`, classes `ej-*` (tokens), portage i18n web fr/en, 4 tests Vitest. **Consolidation** : renommé « Exposition graduée » (affichage ; id interne `fear_thermometer` conservé) + **module `exposure_hierarchy` retiré** (code + base ; 4 patients migrés vers `fear_thermometer`, sources réaffectées). Spec : [`docs/spec/exposure-journey.md`](docs/spec/exposure-journey.md)
