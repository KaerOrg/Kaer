...
- ⛔ **« Suggérer un thème » retiré du front** (décision produit, #122, 2026-07-06) : le bouton + modale du picker, le composant `ThemeSuggestionButton`, le service `themeSuggestionService` et les clés i18n `patient.psycho_suggest*` ont été supprimés. La table `theme_suggestions` et l'Edge Function `send-theme-suggestion` restent en place (infra, non exposée par le front). Historique de la fonctionnalité (décision produit 2026-06-11) conservé ici pour mémoire.
- ✅ **Picker : aucune fiche pré-cochée** (#122) : à l'ouverture en mode « débloquer », la sélection démarre vide ; le praticien coche explicitement les fiches à débloquer (le mode « modifier » reste initialisé sur les fiches déjà débloquées).
