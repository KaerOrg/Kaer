...
| **5 — Finitions** | ✅ **lien fiche psyedu** (7 fiches chrono rattachées au thème `lifestyle` → bibliothèque Psychoéducation) ; ✅ **rappels horaires fixes** (cloche `ChronobiologyCard` → `NotificationRoutineModal`, non conditionnel aux données, MDR-safe). Reste optionnel : streak de saisie ; dette i18n EN/teen du contenu `chrono_bio` | module livrable |
