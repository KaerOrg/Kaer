...
| **5 — Finitions** | ✅ **lien fiche psyedu** (7 fiches chrono rattachées au thème `lifestyle` → bibliothèque Psychoéducation) ; ✅ **rappels horaires fixes** (cloche `ChronobiologyCard` → onglet Notifications de la modale d'actions, `NotificationRoutinePanel`, non conditionnel aux données, MDR-safe). ✅ i18n EN/teen soldée. Reste optionnel : streak de saisie | **module release-ready** |
