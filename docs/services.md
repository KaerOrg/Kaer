...
| [`crisisPlanService.ts`](../apps/web/src/services/crisisPlanService.ts) | Ce que le praticien pose autour du plan : `fetchCrisisPlanConfig`, `saveCrisisPlanConfig` (message affiché dans le widget `crisis_anchors_preview`, « Mes raisons de tenir ») et `markPlanReviewedToday` (PW-5) qui date la revue du jour. **`saveCrisisPlanConfig` n'écrit aucune des deux dates** : enregistrer un message n'est pas une revue. `markPlanReviewedToday` est **idempotent dans la journée** (jour local via `toLocalIsoDate`, jamais `toISOString()`) et ne pose `created_with_at` qu'à la première revue. |
| [`safetyPlanItemsService.ts`](../apps/web/src/services/safetyPlanItemsService.ts) | Items du plan de sécurité côté praticien : `fetchSafetyPlanItems`, `saveSafetyPlanItem`, `deleteSafetyPlanItem`. Écrit dans `safety_plan_items`, la table co-éditée, avec `authored_by = 'practitioner'`. Lecture cachée par `crisisQueries.planItems`. `kind` est typé en union fermée (`PlanItemKind`, `@kaer/shared`) : la colonne est un `text` en base, la lecture passe par `parsePlanItemKind`, une vérification à l'exécution, **jamais un `as`**. L'éditeur des six étapes (PW-2/PW-3) se construit dessus. |
...
| [`crisisPlanService.ts`](../apps/mobile/src/services/crisisPlanService.ts) | Plan de crise patient : ancres photos (`getAnchors`, `pickAndSaveAnchorPhoto` avec permission via `permissionsService`, `removeAnchorPhoto`, `getAnchorPhrase`, `saveAnchorPhrase`), config praticien (`fetchPractitionerConfig` : message de soutien **et** les deux dates de la relation autour du plan, `createdWithAt` / `lastReviewedAt`, qui alimentent l'en-tête de la vue Consultation, P-4). |
