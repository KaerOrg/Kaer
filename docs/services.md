...
| [`crisisPlanService.ts`](../apps/mobile/src/services/crisisPlanService.ts) | Plan de crise patient : ancres photos (`getAnchors`, `pickAndSaveAnchorPhoto`, `removeAnchorPhoto`, `getAnchorPhrase`, `saveAnchorPhrase`), config praticien (`fetchPractitionerConfig`). |
