...
| [`moduleAssignmentService.ts`](../apps/web/src/services/moduleAssignmentService.ts) | Déblocage/révocation de modules, configuration psychoéducation/RIM, proposition d'échelle, effets suivis (`fetchTrackedEffects`/`updateTrackedEffects`), liste de médicaments (`fetchMedications`/`updateMedications`), techniques de défusion proposées (`fetchDefusionTechniques`/`updateDefusionTechniques`, `config.enabled_techniques`, #201) |
...
| [`moduleCatalogService.ts`](../apps/web/src/services/moduleCatalogService.ts) | Catégories + modules pour l'armoire thérapeutique et le formulaire d'invitation ; `fetchModuleTaxonomy` (axes/tags/liaisons `tag_dimensions`+`tags`+`module_tags` pour les filtres par facettes — [`spec/module-taxonomy.md`](spec/module-taxonomy.md)) ; `fetchHiddenModuleIds` (modules masqués, droits non acquis, #247) ; `fetchDormantModules` (modules masqués **avec leur motif**, #421 : la seule lecture qui les récupère volontairement, pour la zone « En veille » du praticien — informative, rien n'y est assignable). **Toutes les autres lectures partant de `modules` filtrent `is_hidden = false`** : le filtre est posé côté requête pour qu'aucun appelant ne puisse l'oublier ([`database.md`](database.md) § `modules`) |
...
| [`engagementService.ts`](../apps/web/src/services/engagementService.ts) | **Graphes d'évolution patient** (`PatientEvolutionTab`) : `fetchScaleEvolution` / `fetchMoodEvolution` / `fetchFearEvolution` / `fetchMedSideEffectsEvolution` / `fetchAvailableScales` — lit **`patient_entries.payload`** (filtré par RLS selon `patients.share_consent`). **Panneau « Données » d'une card module** (`PatientModulesTab` → `ModuleDataPanel`) : graphe d'évolution pour les modules à séries temporelles (mêmes fetchers que ci-dessus + `fetchDefusionEvolution` → `DefusionPoint[]` pour « Décrocher d'une pensée », #201), sinon `fetchModuleSummary(patientId, moduleType)` → `{ lastDate, count, lastPayload }` (filtre par `module_id` seul, tri desc) rendu en tableau (`ModuleSummaryPanel`). **Modules `column_form`** (colonnes de Beck…) : `fetchFormEntries(patientId, moduleId)` → `FormEntryRow[]` (`{ date, values }` bruts depuis `entry_kind='form_entry'`) rendu par `ColumnFormDataPanel` (courbes de tous les curseurs + fiches complètes antichronologiques, structure dérivée de `module_content_fields`). **Module « Nommer ce que je ressens »** : `fetchEmotionNamingEntries(patientId)` → `EmotionNamingRow[]` (une saisie = famille/nuance/mot en **clés i18n**, intensité, contexte, contexte libre, note). Le service LIT seulement : les comptages descriptifs vivent dans les fonctions pures de [`lib/emotionNamingData.ts`](../apps/web/src/lib/emotionNamingData.ts) et la traduction reste aux composants. Ce module n'a **pas de courbe** : ni score de granularité (ICC non calculable sur un seul terme par saisie), ni émodiversité, ni moyenne d'intensité toutes familles confondues. Affichage passif, zéro interprétation (MDR). **Détail d'une passation d'échelle** (#418) : `fetchScalePassations(patientId, moduleType)` → `ScalePassation[]` (`{ id, date, answers, totalScore }`, ordre chronologique) rendu par `ScalePassationsPanel` → `ScalePassationDetail`. Le service ne lit **aucun libellé** : les items vivent dans `module_content_fields` et se résolvent par leur clé i18n au rendu, faute de quoi le web divergerait de l'écran patient à la première correction de traduction. `totalScore` est repris tel que le mobile l'a calculé, jamais recalculé : le praticien doit voir le chiffre que le patient a vu. **Accusé de lecture** (#419) : `markPassationRead(patientId, localId, moduleId)` appelle le RPC `mark_entry_read` (pose la date de PREMIÈRE ouverture, idempotent) **et** trace la consultation au journal d'audit (`scope: 'passation'`). Deux effets volontairement non fusionnés : l'audit enregistre chaque consultation, l'accusé ne retient que la première. Un échec du RPC part à la télémétrie (`reportFailedOperation`) sans rien afficher : un accusé qui ne se pose pas est invisible côté praticien, et le patient attendrait un « Vu le … » qui ne vient jamais. |
...
| [`crisisPlanService.ts`](../apps/web/src/services/crisisPlanService.ts) | Config plan de crise praticien : `fetchCrisisPlanConfig`, `saveCrisisPlanConfig` — message praticien affiché dans le widget `crisis_anchors_preview` (« Mes raisons de tenir ») de la vue `safety_plan`. |
| [`errorReportingService.ts`](../apps/web/src/services/errorReportingService.ts) | `reportAppError`, `reportFailedOperation`, `installGlobalErrorHandlers` — alerte email sur crash / opération échouée via l'Edge Function `report-app-error` (**fire-and-forget**). Voir [`app-error-alerting.md`](app-error-alerting.md). |
...
| [`scaleScheduleService.ts`](../apps/web/src/services/scaleScheduleService.ts) | Programmation des auto-questionnaires (K-6/K-7) : `fetchScaleSchedule` (une échelle, encart), `fetchScaleSchedules` (toutes les échelles du patient, colonne « Programmée »), `saveScaleSchedule` (upsert sur `(patient_id, module_id)`), `deleteScaleSchedule`. Table `scale_schedules`. Query : `scaleScheduleQueries.byPatient` / `byScale` (invalidation par le préfixe patient). **MDR** : cadence choisie par le praticien, aucune relance liée à un score. L'état « en retard » est dérivé de la date d'échéance par `lib/scaleScheduleStatus.ts` (`computeScheduleStatus`) : un pur fait de calendrier, jamais un score. **`instruction`** (#421) porte la consigne affichée au patient, deux phrases au plus, restituée telle quelle : un champ vidé est enregistré à `null`, pour qu'« aucune consigne » ait une seule représentation en base. |
...
| [`permissionsService.ts`](../apps/mobile/src/services/permissionsService.ts) | **Autorisations système génériques** : `checkPermission`/`requestPermission`/`ensurePermission` sur un `PermissionKind` (`notifications`, `contacts`, `mediaLibrary`, `camera`). Point d'entrée unique délégant aux modules Expo, statut normalisé. Utilisé par `notificationService`, `avatarService`, `crisisPlanService`, `contactsService`. Chaînes d'explication iOS (`Info.plist`) déclarées dans `app.json`. |
| [`contactsService.ts`](../apps/mobile/src/services/contactsService.ts) | `pickContact` — présente le picker natif du répertoire (`expo-contacts`) et renvoie nom + premier numéro. iOS : aucune permission requise (picker hors-process) ; Android : `READ_CONTACTS` via `permissionsService`. Import d'un contact appelable dans le plan de crise (étapes 4 & 5). |
| [`notificationService.ts`](../apps/mobile/src/services/notificationService.ts) | Planification des rappels (stubs Expo Go SDK 53+), `getAllRoutinesForPatient`, `pauseRoutine`/`resumeRoutine` (la pause écrit un événement dans `notification_events`). Permissions déléguées au `permissionsService`. |
| [`avatarService.ts`](../apps/mobile/src/services/avatarService.ts) | Picker image (permissions via `permissionsService`), upload Supabase Storage, mise à jour `patients.avatar_url` |
| [`errorReportingService.ts`](../apps/mobile/src/services/errorReportingService.ts) | `reportAppError`, `flushAppErrorOutbox`, `reportFailedOperation`, `installGlobalErrorHandlers` — alerte email sur crash / opération échouée (offline-first : file SQLite `app_error_outbox` drainée au foreground). **Télémétrie technique, pas de donnée patient** → exception légitime à `syncHelpers`. Voir [`app-error-alerting.md`](app-error-alerting.md). |
...
| [`scaleScheduleService.ts`](../apps/mobile/src/services/scaleScheduleService.ts) | `fetchScaleSchedule`, `fetchScaleSchedules` : **lecture seule** de la programmation posée par le praticien (`scale_schedules`, #413). Miroir mobile du service web du même nom, qui porte le CRUD : la RLS n'accorde au patient qu'un `select` sur ses lignes (`scale_schedules_patient_select`). Le patient ne modifie pas la cadence ; ses ajustements (heure, jours, pause) vivent dans `notification_routines` via `notificationService` et remontent au praticien par ce canal. Depuis #421, la lecture rapporte aussi `instruction` : la consigne du praticien, affichée telle quelle sur l'écran de premier lancement du module (une consigne vide vaut aucune consigne, pas de ligne fantôme). **MDR** : données de programmation uniquement, aucune réponse ni score, aucun déclenchement. |
| [`scaleDraftService.ts`](../apps/mobile/src/services/scaleDraftService.ts) | `loadDraft`, `saveDraft`, `discardDraft`, `purgeStaleDrafts` : saisie d'échelle EN COURS, gardée sur le téléphone et nulle part ailleurs (#412). **Un brouillon n'est pas une passation** : une saisie inachevée n'a pas de score, ne se compare à rien, et n'est pas ce que le patient a choisi de transmettre → **exception légitime à `syncHelpers`**, justifiée en JSDoc. Zéro `EntryKind`, zéro `sync_outbox`, zéro appel réseau (vérifié par test). Purge à l'envoi et au bout de `DRAFT_MAX_AGE_DAYS` (7 jours), au delà desquels la fenêtre de deux semaines a glissé. |
| [`scaleReceiptService.ts`](../apps/mobile/src/services/scaleReceiptService.ts) | `fetchScaleReadReceipts(scaleId)` : **lecture seule** des accusés de lecture des passations (`patient_entries.practitioner_read_at`, #419), indexés par identifiant local. Alimente la ligne « Vu par … le … » du dernier envoi. **Exception structurelle à `syncHelpers`** : cette donnée n'est pas produite par le patient et la base lui interdit de l'écrire, il n'y a donc rien à mettre en file de sortie. Une passation jamais ouverte est absente de la map, et un échec de lecture rend une map vide : hors ligne, on n'affiche rien plutôt qu'un accusé inventé. |
...
| [`treeSelectionService.ts`](../apps/mobile/src/services/treeSelectionService.ts) | `saveTreeSelection`, `deleteTreeSelection` : sélecteurs hiérarchiques (« Nommer ce que je ressens »). `saveTreeSelection` accepte un `created_at` optionnel : en modification, il préserve l'horodatage d'origine, que `INSERT OR REPLACE` réinitialiserait sinon |
| [`moduleOnboardingService.ts`](../apps/mobile/src/services/moduleOnboardingService.ts) | `hasSeenModuleOnboarding`, `markModuleOnboardingSeen` : mémoire locale des écrans d'introduction vus une fois. **Non synchronisé volontairement** : état d'interface, pas donnée de soin (exception documentée de [`sync-service.md`](../.claude/rules/sync-service.md)) |
| [`planItemService.ts`](../apps/mobile/src/services/planItemService.ts) | `getPlanItems` (lecture), `savePlanItem`, `deletePlanItem`, `setModuleSetting` — plans éditables + settings module. Un item porte un `phone`/`contact_source` optionnels (contacts appelables du plan de crise). |
...
| [`breathingService.ts`](../apps/mobile/src/services/breathingService.ts) | `fetchBreathingTechniques` (config-first, lue en base), `fetchBreathingSessions`, `saveBreathingSession` (sessions menées au bout OU interrompues, payload enrichi + `client_created_at`=`started_at`), `fetchBreathingSettings` / `saveBreathingSettings` (config par patient, `entry_kind='breathing_setting'`, `local_id` stable) — techniques de respiration (T0 #195) |
...
| [`appointmentService.ts`](../apps/mobile/src/services/appointmentService.ts) | Lecture des RDV patient (`fetchPatientAppointments`), règles de disponibilité praticien (`fetchPractitionerRules`, `fetchPractitionerExceptions`) pour affichage dans `TodaySchedule`, identité du soignant suivi (`fetchMyPractitioner`, via la RPC `get_my_practitioner` : la policy `practitioners_own` interdit la lecture directe côté patient). Fonctions pures : `computeAvailableSlots`. |
| [`crisisPlanService.ts`](../apps/mobile/src/services/crisisPlanService.ts) | Plan de crise patient : ancres photos (`getAnchors`, `pickAndSaveAnchorPhoto` — permission via `permissionsService`, `removeAnchorPhoto`, `getAnchorPhrase`, `saveAnchorPhrase`), config praticien (`fetchPractitionerConfig`). |
...
| [`profileStatsService.ts`](../apps/mobile/src/services/profileStatsService.ts) | `fetchProfileStats(patientId)` — résumé de suivi du hub Profil : `created_at` (ancienneté), nombre de modules débloqués, nombre de séances honorées (`appointments` statut `completed`), en comptages `head: true`. Valeurs brutes, descriptives (MDR). Helpers purs d'affichage : `lib/profileStats.ts`. |
...
| [`languageService.ts`](../apps/mobile/src/services/languageService.ts) | Persistance locale de la langue d'affichage (AsyncStorage, clé `kaer.language`) : `loadStoredLanguage`, `persistLanguage`, `isSupportedLanguage`. Préférence d'interface, aucune donnée patient : exception légitime à `syncHelpers`. Consommé par `authStore.restoreLanguage` (boot) et `authStore.setLanguage` (profil). |
