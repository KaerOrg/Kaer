...
| [`scaleScheduleService.ts`](../apps/web/src/services/scaleScheduleService.ts) | Programmation des auto-questionnaires (K-6) : `fetchScaleSchedule`, `saveScaleSchedule` (upsert sur `(patient_id, module_id)`), `deleteScaleSchedule`. Table `scale_schedules`. **MDR** : cadence choisie par le praticien, aucune relance liée à un score. |
