...
| [`breathingStats.ts`](../packages/shared/src/services/breathingStats.ts) | Helper PUR (pas de client) des sessions de respiration : `buildWeekPractice` (7 jours lundi → dimanche), `sessionsInWeek`, `currentStreak` (jours consécutifs, la série reste ouverte tant que la journée en cours n'est pas terminée), `lastSession`. Contrat structurel minimal (`{ date }`, + `started_at`) : la ligne SQLite mobile comme la ligne Supabase le satisfont. MDR : comptes bruts, jamais de qualification. | [`breathingStats.test.ts`](../packages/shared/src/services/breathingStats.test.ts) |
...
| [`breathingService.ts`](../apps/mobile/src/services/breathingService.ts) | `fetchBreathingTechniques` (config-first, lue en base), `fetchBreathingSessions`, `saveBreathingSession` (sessions menées au bout OU interrompues, payload enrichi + `client_created_at`=`started_at`), `fetchBreathingSettings` / `saveBreathingSettings` (config par patient, `entry_kind='breathing_setting'`, `local_id` stable) : techniques de respiration (T0 #195). **Purs** : `techniquesFromFields`, `getCycleDuration`, `breathingConfigFromFields` (lit `bt.config`), `resolveActivation` (ne rend que les techniques activées par le praticien ; repli sur `default_technique_key` tant que `enabled_techniques` est vide, M1 #366), `techniqueInfoFromFields` (blocs « En savoir plus » lus en base, M6 #371). |
| [`breathingReminderService.ts`](../apps/mobile/src/services/breathingReminderService.ts) | Rappels **locaux** du module respiration (M5 #195) : `syncBreathingReminders` (annule puis reprogramme, donc jamais de doublon ; marqueur `kind: 'breathing_reminder'` pour ne toucher qu'aux siennes), `cancelBreathingReminders`. Permission demandée seulement si un rappel est réellement voulu. **MDR : titre et corps reçus déjà traduits, aucune interpolation possible, aucune donnée de pratique lue** (garde-fou `__tests__/breathingReminderText.guard.test.ts`). Plan pur : [`lib/breathingReminderPlan.ts`](../apps/mobile/src/lib/breathingReminderPlan.ts). |
