...
| [`safetyPlanItemsService.ts`](../apps/web/src/services/safetyPlanItemsService.ts) | Items du plan de sécurité côté praticien : `fetchSafetyPlanItems`, `saveSafetyPlanItem`, `deleteSafetyPlanItem`. Écrit dans `safety_plan_items`, la table co-éditée, avec `authored_by = 'practitioner'`. Lecture cachée par `crisisQueries.planItems`. L'éditeur des six étapes (PW-2) se construit dessus. |
...
| [`planItemService.ts`](../apps/mobile/src/services/planItemService.ts) | `getPlanItems` (lecture), `savePlanItem`, `deletePlanItem`, `setModuleSetting` — plans éditables + settings module. **Strictement SQLite**, verrouillé par un test statique : c'est ce qui garantit que la Séquence du plan de sécurité fonctionne hors ligne. Sert la Balance décisionnelle et les settings ; le plan de sécurité, lui, passe par `safetyPlanService`. |
| [`safetyPlanService.ts`](../apps/mobile/src/services/safetyPlanService.ts) | `refreshSafetyPlan`, `saveSafetyPlanItem`, `deleteSafetyPlanItem` — le plan de sécurité est un **document co-édité** (`safety_plan_items`), pas une saisie : il s'écrit sur le serveur puis dans le cache local, et ne passe jamais par l'outbox montante. Séparé de `planItemService` pour que le réseau reste hors du chemin de lecture de la Séquence. |
