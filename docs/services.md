...
| [`crisisPlanService.ts`](../apps/web/src/services/crisisPlanService.ts) | Config plan de crise praticien : `fetchCrisisPlanConfig`, `saveCrisisPlanConfig` — message praticien affiché dans le widget `crisis_anchors_preview` (« Mes raisons de tenir ») de `EditableStepsLayout`. |
